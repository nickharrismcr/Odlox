...
// AST-shape tests for implementation phase 2 of docs/plans/compiler-ast-
// split.md: the Pratt-to-AST parser for expressions. compile_test.odin's
// opcode-sequence assertions can't apply yet -- no bytecode exists until
// Emit (phase 5) -- so these check tree shape instead, the same role
// compile_test.odin's decode() plays for the old pipeline.
...
// and AST nodes returned by expression_ast hold onto raw Tokens whose
...
@(private = "file")
parse_expr_ast :: proc(t: ^testing.T, source: string) -> Expr {
	scn := tokenize(source)
...
test_ast_property_get_set_invoke_shape :: proc(t: ^testing.T) {
	...
	invoke := parse_expr_ast(t, "obj.method(1)")
