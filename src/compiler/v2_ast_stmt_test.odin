...
// AST-shape tests for implementation phase 3 of docs/plans/compiler-ast-
// split.md: statement/declaration parsing. Same rationale as
// v2_ast_expr_test.odin -- compile_test.odin's opcode assertions can't
// apply until Emit (phase 5), so these check tree shape instead.

// destroy_scanner is deliberately not called here -- see
// v2_ast_expr_test.odin's parse_expr_ast for why.
@(private = "file")
parse_program_ast :: proc(t: ^testing.T, source: string) -> []Stmt {
	scn := tokenize(source)
...
// parse_one_ast is parse_program_ast for the common single-statement case.
@(private = "file")
parse_one_ast :: proc(t: ^testing.T, source: string) -> Stmt {
	stmts := parse_program_ast(t, source)
...
test_ast_destructuring_shape :: proc(t: ^testing.T) {
	single_rhs := parse_one_ast(t, "a, b = pair()")
	...
	multi_rhs := parse_one_ast(t, "a, b = 1, 2")
...
test_ast_if_else_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "if (a) print 1\nelse print 2")
	i, ok := s.(^Stmt_If)
	...
	no_else := parse_one_ast(t, "if (a) print 1")
...
test_ast_while_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "while (a) print 1")
	w, ok := s.(^Stmt_While)
...
test_ast_for_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "for (var i = 0; i < 10; i = i + 1) { print i }")
	f, ok := s.(^Stmt_For)
...
test_ast_for_bare_init_uses_implicit_assign :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "for (i = 0; i < 10; i = i + 1) { print i }")
	f, ok := s.(^Stmt_For)
	testing.expect(t, ok, "expected Stmt_For")
...
test_ast_foreach_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "foreach (x in list) { print x }")
	f, ok := s.(^Stmt_Foreach)
...
test_ast_break_continue_shape :: proc(t: ^testing.T) {
	...
	b := parse_one_ast(t, "break")
	_, b_ok := b.(^Stmt_Break)
	testing.expect(t, b_ok, "expected Stmt_Break")

	c := parse_one_ast(t, "continue")
...
test_ast_function_declaration_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "func add(a, b=1, *rest) { return a + b }")
...
test_ast_lambda_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "var f = func(x) { return x }")
...
test_ast_class_declaration_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "class Dog < Animal {\ninit(name) { this.name = name }\nstatic count = 0\nbark() { print \"woof\" }\n}")
...
test_ast_self_inheritance_is_still_flagged :: proc(t: ^testing.T) {
	...
	p := Parser{scn = &scn, filename = "test.lox"}
	advance(&p)
	advance(&p) // consume 'class', as declaration_ast's own dispatch does before calling this
...
test_ast_try_without_except_or_finally_is_error :: proc(t: ^testing.T) {
	...
	p := Parser{scn = &scn, filename = "test.lox"}
	advance(&p)
	advance(&p) // consume 'try', as statement_ast's own dispatch does before calling this
	try_except_statement_ast(&p)
...
test_ast_import_shape :: proc(t: ^testing.T) {
	s := parse_one_ast(t, "import math, sys as system")
...
test_ast_from_import_shape :: proc(t: ^testing.T) {
	named := parse_one_ast(t, "from math import sqrt, pow")
	...
	wildcard := parse_one_ast(t, "from math import *")
