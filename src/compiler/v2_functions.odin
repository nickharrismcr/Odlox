...
// AST-building counterpart to functions.odin's compile_function_body --
// see v2_parser.odin's header comment for why this is `_ast`-suffixed.
// Shared by declared functions (v2_stmt.odin's function_declaration_ast),
// lambdas (v2_expr.odin's lambda_ast), and class methods (v2_stmt.odin's
// method_ast), exactly as compile_function_body is shared by all three
// today. Only the parsing half survives here -- arity/min_arity are
// derivable from `params` at Emit time, so unlike compile_function_body
// this doesn't need to track them; scope/local declaration for each
// parameter is the Resolver's job (fills Param.declared_slot later).
parse_function_decl_ast :: proc(p: ^Parser, fn_type: Function_Type, name_tok: Token) -> ^Function_Decl {
