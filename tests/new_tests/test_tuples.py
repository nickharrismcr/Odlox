...
def test_tuples(script):
    ...
    # tuple concatenation
    assert lines[7] == "[ 1 , 2 , 3 , 7 , 8 , 9 ]"
    # append(2, a) raises RuntimeError — tuples are immutable (append expects list as first arg).
    # lines[8], not lines[-1]: odlox now prints a call stack trace after the
    ...
    assert "Uncaught exception" in lines[8]
    assert "append" in lines[8].lower() or "list" in lines[8].lower()
